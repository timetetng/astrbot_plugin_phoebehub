...
Pure async, no AstrBot dependency. Tested via `python -m phoebehub_pr --selftest`.
"""
...
def _selftest() -> None:
    """Minimal smoketest that builds entries from sample sidecar data."""
    sample = {
        "name": "开心菲比", "fmt": "webp",
        "original_bytes": 123456, "final_bytes": 23456,
        "width": 400, "height": 300,
        "uploaded_by": "test", "uploaded_at": 1712345678,
        "source_filename": "test.png", "note": "",
        "ai_description": "一只开心的菲比",
        "user_hint": "开心",
    }
    entry = build_entry(sample)
    assert entry.title == "开心菲比"
    assert entry.filename == "开心菲比.webp"
    assert not entry.is_gif
    assert entry.description == "一只开心的菲比"

    assert _next_id([]) == 1
    assert _next_id([{"id": 5}, {"id": 3}]) == 6

    gif_sample = {**sample, "fmt": "gif"}
    gif_entry = build_entry(gif_sample)
    assert gif_entry.filename == "开心菲比.gif"
    assert gif_entry.is_gif
...
if __name__ == "__main__":
    ...
    if "--selftest" in sys.argv:
        ...
        token = (sys.argv[sys.argv.index("--token") + 1]
                 if "--token" in sys.argv else "")
        test_owner = (sys.argv[sys.argv.index("--owner") + 1]
                      if "--owner" in sys.argv else "timetetng")

        if not token:
            print("usage: python -m phoebehub_pr --live --token ghp_xxx [--owner timetetng]",
                  file=sys.stderr)
            sys.exit(1)

        async def _live():
            pr = PhoebeHubPR(token)

            with tempfile.TemporaryDirectory() as td:
                td = Path(td)
                dummy = td / "test_菲比_pr.webp"
                dummy.write_text("pr-test-image")
                dummy_json = td / "test_菲比_pr.webp.json"
                dummy_json.write_text(
                    json.dumps({
                        "name": "test_菲比_pr", "fmt": "webp",
                        "original_bytes": 100, "final_bytes": 50,
                        "width": 100, "height": 100,
                        "uploaded_by": "test",
                        "uploaded_at": int(time.time()),
                        "source_filename": "test.png", "note": "",
                        "ai_description": "PR 测试用表情包",
                        "user_hint": "test",
                    }, ensure_ascii=False))

                print("=== Testing submit (push + PR to test_owner repo) ===")
                result = await pr.submit(
                    td, target_owner=test_owner, target_repo="Phoebe-Hub",
                    create_pr=True,
                )
        ...
        if not token:
            print("usage: python -m phoebehub_pr --live-full --token ghp_xxx",
                  file=sys.stderr)
            sys.exit(1)

        async def _live_full():
            pr = PhoebeHubPR(token)

            with tempfile.TemporaryDirectory() as td:
                td = Path(td)
                dummy = td / "test_完整流程.webp"
                dummy.write_text("full-flow-test")
                dummy_json = td / "test_完整流程.webp.json"
                dummy_json.write_text(
                    json.dumps({
                        "name": "test_完整流程", "fmt": "webp",
                        "original_bytes": 100, "final_bytes": 50,
                        "width": 100, "height": 100,
                        "uploaded_by": "test",
                        "uploaded_at": int(time.time()),
                        "source_filename": "test.png", "note": "",
                        "ai_description": "完整流程测试",
                        "user_hint": "test",
                    }, ensure_ascii=False))

                print("=== Testing submit_pr (fork → sync → commit → PR) ===")
                result = await pr.submit_pr(td)
                print(json.dumps(result, ensure_ascii=False, indent=2))

        asyncio.run(_live_full())

    else:
        print("usage: python -m phoebehub_pr "
              "--selftest | --live --token ghp_xxx [--owner user] | --live-full --token ghp_xxx",
              file=sys.stderr)
        sys.exit(1)
